...
function Retreat(RetreatSpace)
	local distanceToDireT = (GetUnitToLocationDistance(npcBot, DIRE_TTOWER1) / 2)
	local distanceToDireB = (GetUnitToLocationDistance(npcBot, DIRE_BTOWER1) / 2)
	local distanceToRadiantT = (GetUnitToLocationDistance(npcBot, RADIANT_TTOWER1) / 2)
	local distanceToRadiantB = (GetUnitToLocationDistance(npcBot, RADIANT_BTOWER1) / 2)
	if (team == 3) then
		if (pID == 7 or pID == 8) then
			MoveTo(npcBot, GetLocation(npcBot) + Vector(distanceToDireT, distanceToDireT, 0))
			return
		elseif (pID == 9 or pID == 10) then
			MoveTo(npcBot, GetLocation(npcBot) + Vector(distanceToDireB, distanceToDireB, 0))
			return
		else
		...
		if (pID == 2 or pID == 3) then
			MoveTo(npcBot, GetLocation(npcBot) + Vector(distanceToRadiantT, distanceToRadiantT, 0))
			return
		elseif (pID == 4 or pID == 5) then
			MoveTo(npcBot, GetLocation(npcBot) + Vector(distanceToRadiantB, distanceToRadiantB, 0))
			return
		else
			MoveDirectly(npcBot, GetLocation(npcBot) - Vector(RetreatSpace, RetreatSpace, 0))
			return
...
----Counts the number of units (Heroes or creeps)----
...
----If no other actions, then move to lane (first 20 seconds)----
	...
	----Back the fuck out----
		if (percentHealth <= 0.25) then
			BTFO()
			return
		elseif (percentHealth <= 0.50) then
			Retreat(100)
			return
		end
	...
	----Retreat from damage if health is low----
		if (percentHealth <= 0.2) then
			BTFO()
			return
		elseif (percentHealth <= 0.5 or WRDByTower(npcBot, 0.5) or WRDByCreep(npcBot, 0.5) or WRDByHero(npcBot, 0.5)) then
			Retreat(100)
			return
		end
	----If no other actions, move to beginning of farm----
